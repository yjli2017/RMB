...
def run(config: RMBConfig, mt_path: str, pn_path: str) -> Dict:
    ...
    ax.plot(x, pop_sleep, color=NATURE_PALETTE["purple"], linewidth=1.15)
    ax.fill_between(x, pop_sleep, alpha=0.20, color=NATURE_PALETTE["purple"], lw=0)
    ...
    plt.close(fig)
    pf = go.Figure(go.Scatter(x=x, y=pop_sleep, mode="lines",
                              line={"color": NATURE_PALETTE["purple"], "width": 2}))
    for b in boundaries:
        pf.add_vline(x=b, line_color=NATURE_PALETTE["grey"], line_dash="dash", opacity=0.4)
    nature_plotly_layout(pf, "Population sleep fraction over time", width=1200, height=420)
    pf.update_yaxes(title="Sleep fraction")
    ...
    im = ax.imshow(sleep.values.T, aspect="auto", cmap="Greys", interpolation="nearest", rasterized=True)
    add_day_boundaries(ax, boundaries, color=NATURE_PALETTE["vermillion"], alpha=0.55)
    ...
    cb = plt.colorbar(im, ax=ax, fraction=0.025, pad=0.015, label="Sleep (0/1)")
    cb.outline.set_visible(False)
    save_nature_figure(fig, p2_png, dpi=config.plot_dpi)
    plt.close(fig)
    pf = go.Figure(go.Heatmap(z=sleep.values.T, colorscale="Greys", showscale=True))
